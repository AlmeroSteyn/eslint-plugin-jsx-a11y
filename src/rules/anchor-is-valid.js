...
const preferButtonErrorMessage = 'Anchor used as a button. ' +
  ...
  'Rather use the button element.';

const noHrefErrorMessage = 'The href attribute is required on an Anchor. ' +
  'Provide a valid navigable address as href.';

const invalidHrefErrorMessage = 'Anchor has an invalid href attribute. ' +
  'The href should not be empty, only contain "#" or contain the word "javascript". ' +
  'Provide a valid navigable address as an href.';
  ...
  create: (context: ESLintContext) => ({
    JSXOpeningElement: (node: JSXOpeningElement) => {
      ...
      const invalidHrefValues = values
        ...
        .filter(value => value === '' || value === '#' || value.toLowerCase().includes('javascript'));
